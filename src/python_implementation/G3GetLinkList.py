...
data = np.loadtxt(path, dtype=str, skiprows=0) #no need to skip anymore
...
path = config.data_path_to_VIM_matrices + "VIM_Federated_3_uneven.npy"
G3 = np.load(path).astype(float)
print("Finished reading F Matrix")

path = config.data_path_to_VIM_matrices + "Federated_3_uneven_linked_list_5000000.txt"
